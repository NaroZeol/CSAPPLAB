...
pid_t stopSign;             /*The stop signal*/
int rReadSignal;            /*The re-read signal to shell*/
...
void eval(char *cmdline) 
{
    ...
    bg = parseline(cmdline, argvArray);

    if (builtin_cmd(argvArray))// builtin_cmd finish, return to main routine
        return;
    
    pid_t pid;
    sigset_t mask, prev;        /*The sigset value*/
    sigemptyset(&mask);
...
int builtin_cmd(char **argv) 
{
    ...
    else if (strcmp("quit", argv[0]) == 0){//exit shell
        exit(0);
    }
    else if (strcmp("jobs", argv[0]) == 0){//list jobs
        listjobs(jobs);
        return 1;
    }
...
*/
    ...
    else if (argv[1] != NULL && ('0' <= (argv[1][0]) && argv[1][0] <= '9')){
        mode = 0;
        id = atoi(argv[1]);
    }
    else{
